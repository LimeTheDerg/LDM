...
int main() {
    ...
    while (true) {
        char buffer[1024]; // Create the buffer
        const ssize_t bytes_read = read(fifo_stream, buffer, 1024); // Read from the fifo
        buffer[bytes_read] = '\0'; // Null terminate
        if (bytes_read > 0) {
            cout << buffer << std::flush; // Flush is mandatory, printing will not wirk without it
        }
    }
    return 0;
}